				...
				<tbody>
					<tr v-for="quota in quotas" :key="quota.id">
						<td class="border border-gray-100 p-2">
							...
							<a :href="`https://api.ipoinbox.com/${ipo.anchors_pdf}`" target="_blank" v-if="quota.cat.id === 6" class="font-medium rounded-lg text-sm text-center inline-block">
				...
				<Tab v-for="(subs, i) in subscriptions" :key="i" :name="formatDate(subs.items[0].day)" :title="formatDate(subs.items[0].day)">
					...
					<tbody>
						<tr v-for="item in subs.items" :key="item.id">
							...
							<td class="border border-gray-400 p-2">{{ (item.subscription !== null) ? (item.subscription / item?.quota).toFixed(2)+'x' : 'NA' }}<span v-if="item?.applications"> (
								<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="w-4 h-4 inline">
	<path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
  </svg>{{ item?.applications }})</span></td>
						</tr>
						<tr>
