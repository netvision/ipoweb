	...
	<div class="flex justify-center mt-8">
		<div class="flex flex-wrap w-3/4">
			<div v-for="ipo in ipos" :key="ipo.ipo_id">
				<button class="bg-gray-200 rounded-lg shadow-md p-4 m-2" @click="goTo(ipo.ipo_id)">
					{{ ipo.company_name }}
				</button>
			</div>
		</div>
	...
	<div class="flex justify-center mt-6">
		<div class="w-3/4">
				...
				<div class="absolute top-15 left-3 p-3 z-10 w-1/2" v-if="searchedItems">
					<div class="w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
						<div  v-for="item in searchedItems" :key="item.ipo_id">
							<a :href="'ipo/'+item.ipo_id" aria-current="true" class="block w-full px-4 py-2 m-2 text-gray-900 border-b border-gray-200 rounded-t-lg cursor-pointer dark:bg-gray-800 dark:border-gray-600">
        						{{ item.company_name }}
    						</a>
						</div>
					</div>
			...
			<div class="mt-2 text-left">
				<a href="#" class="text-gray-500 text-sm underline">or checkout our IPO Archive</a>
			</div>
		</div>
	</div>
